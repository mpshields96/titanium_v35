        ...
        for game in games:
            ...
            for market in dk_book.get('markets', []):
                ...
                if sport == "NHL":
                    if market['key'] == 'h2h':
                         for outcome in market['outcomes']:
                            if -135 <= outcome['price'] <= 145:
                                sort_val = 80 - abs(outcome['price'] + 110)
                                candidates.append({"Sport": "NHL", "Time": time_str, "Matchup": matchup, "Type": "Moneyline", "Target": outcome['name'], "Line": "ML", "Price": outcome['price'], "Book": dk_book['title'], "Audit_Directive": "Goalie Check.", "Sort_Val": sort_val})
                    
                    elif market['key'] == 'spreads': # Puck Line
                        for outcome in market['outcomes']:
                            if -150 <= outcome['price'] <= 160:
                                sort_val = 70
                                if outcome['price'] > 100: sort_val = 85 # PL Value
                                candidates.append({"Sport": "NHL", "Time": time_str, "Matchup": matchup, "Type": "Puck Line", "Target": outcome['name'], "Line": outcome['point'], "Price": outcome['price'], "Book": dk_book['title'], "Audit_Directive": "Safety Valve.", "Sort_Val": sort_val})
                ...
                # SOCCER LOGIC
                    ...
                    if market['key'] == 'h2h':
                        for outcome in market['outcomes']:
                             if -130 <= outcome['price'] <= 180:
                                sort_val = 70
                                # FILE 002 LEGACY: Chaos Mode
                                if outcome['name'] == "Draw": sort_val = 85
                                candidates.append({"Sport": "SOCCER", "Time": time_str, "Matchup": matchup, "Type": "3-Way", "Target": outcome['name'], "Line": "ML", "Price": outcome['price'], "Book": dk_book['title'], "Audit_Directive": "Verify xG.", "Sort_Val": sort_val})
